...
- **矩阵/向量**：必须使用单行格式，元素间用空格分隔，不要使用换行符
  - 正确格式：`$\begin{pmatrix} a & b & c & d \end{pmatrix}$`（单行，空格分隔）
  - 错误格式：`$\begin{pmatrix} a & b \\ c & d \end{pmatrix}$`（含换行符，会渲染失败）
  - 对于多行矩阵，改用：`$\left[\begin{smallmatrix} a & b \\ c & d \end{smallmatrix}\right]$` 或直接写成向量形式
